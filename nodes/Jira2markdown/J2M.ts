...
export class J2M {
	 ...
	 * @returns {string} The HTML result
	 */
	static jira_to_html(str: string) {
		return marked.parse(J2M.to_markdown(str), { async: false });
	}
		...
		return (
			str
				...
				.replace(
					...
					(match, from: 'del' | 'ins' | 'sup' | 'sub', content) => {
						const to = map[from];
